...
#마감시간 로직 백엔드 api 코드
@app.route("/api/check_meal_eligibility")
def check_meal_eligibility():
    """
    희망 날짜(target_date) 식사 신청 가능 여부 확인 API
    조건:
      - 희망 날짜 주차의 1주 전 수요일 16:00 이전 (<)
      - 희망 날짜 주차의 N주 전 월요일 0:00 이후 (>)
      - created_at 기준으로 희망 날짜 식사 OR selfcheck 1건 이상
    """
    user_id = request.args.get("user_id")
    target_date_str = request.args.get("target_date")   # YYYY-MM-DD
    past_weeks = int(request.args.get("past_weeks", 2)) # 하한 주차 (기본 2)

    if not user_id or not target_date_str:
        return jsonify({"error": "user_id와 target_date는 필수입니다."}), 400

    target_date = datetime.strptime(target_date_str, "%Y-%m-%d")

    # ① 희망 날짜 주차의 월요일 0:00
    monday_of_target_week = target_date - timedelta(days=target_date.weekday())
    monday_of_target_week = monday_of_target_week.replace(hour=0, minute=0, second=0, microsecond=0)

    # ② 1주 전 수요일 16:00
    monday_1w_before = monday_of_target_week - timedelta(weeks=1)
    wednesday_1w_before_16 = (monday_1w_before + timedelta(days=2)).replace(
        hour=16, minute=0, second=0, microsecond=0
    )

    # ③ N주 전 월요일 0:00
    monday_past_weeks = (monday_of_target_week - timedelta(weeks=past_weeks)).replace(
    ...
    """, (
        user_id,
        target_date_str,
        monday_past_weeks.strftime("%Y-%m-%d %H:%M:%S"),
        wednesday_1w_before_16.strftime("%Y-%m-%d %H:%M:%S")
    ))
    meal_count = cur.fetchone()[0]

    # selfcheck 테이블 확인
    cur.execute("""
        SELECT COUNT(*) FROM selfcheck
        WHERE user_id = ?
          AND date = ?
          AND created_at > ?
          AND created_at < ?
    """, (
        user_id,
        monday_of_target_week.strftime("%Y-%m-%d"),
        monday_past_weeks.strftime("%Y-%m-%d %H:%M:%S"),
        wednesday_1w_before_16.strftime("%Y-%m-%d %H:%M:%S")
    ))
    selfcheck_count = cur.fetchone()[0]

    conn.close()

    eligible = (meal_count > 0) or (selfcheck_count > 0)
